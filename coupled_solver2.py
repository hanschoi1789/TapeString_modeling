...
print("sanity rem(nat,0)=", f_rem(0.95,0,0,0,0.0), " TH(nat,0)=", f_TH(0.95,0,0,0.0), flush=True)

beta0=0.95; L=200.0
bcL=(beta0,'tight'); bcR=(beta0,'tight')

def rhs(s,y,p):
    Mv=p[0]
    y4=-f_rem(y[0],y[1],y[2],y[3],Mv)/f_c4(y[0],y[1],y[2],y[3],Mv)
    th=f_TH(y[0],y[1],y[2],Mv)
    return np.vstack([y[1],y[2],y[3],y4,th])
...
x=np.linspace(0,L,121)
y=np.zeros((5,x.size)); y[0]=beta0
p=np.array([0.0])
results=[]
# coarser sweep, tighter budget
th_list=np.concatenate([np.linspace(0.005,0.12,10), np.linspace(0.14,0.5,19)])
t_deadline=time.time()+230
for thL in th_list:
    if time.time()>t_deadline:
        print("time budget reached"); break
    sol=solve_bvp(rhs, make_bc(thL), x, y, p=p, max_nodes=60000, tol=1e-5)
    if not sol.success:
        print(f"thL={thL:.4f} FAILED ({sol.message})"); break
...
pickle.dump(results, open('case1_results.pkl','wb'))
