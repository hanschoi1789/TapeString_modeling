...
files = sys.argv[1:] if len(sys.argv) > 1 else ['case_force.pkl']
...
if force is not None and 'Mdist' in force[2][peak_idx(force[2])]:
    ...
    axD.plot(s, Mdist, color=RED, lw=2.4, label='M(s1)=P(L-s1) at limit point')
    if moment is not None:
...
plt.tight_layout()
plt.savefig('force_vs_moment_singleclamp.png', dpi=130)
print("saved force_vs_moment_singleclamp.png")
